...
+(NSString *)stringWithStringAppen:(NSArray *)pathArr{
    NSMutableString *string = [[NSMutableString alloc]init];
    NSString *douhao = @",";
    
    for (int i = 0; i<pathArr.count; i++) {
        NSString *str = [pathArr objectAtIndex:i];
        if (pathArr.count==1) {
            string = [[NSMutableString alloc]initWithString:str];
            break;
        }else{
            [string appendString:str];
            if (pathArr.count-1 == i) {
                break;
            }
            else{
                [string appendString:douhao];
            }
        }
    }
    return string;
...
+ (NSString *)conversion:(NSInteger )fileSize
{
    NSString *f;
    if (fileSize >= 1024 && fileSize < 1048576)
    {
        f = [NSString stringWithFormat:@"%.1fKB", (float)fileSize/1024];
    }
    else if (fileSize >= 1048576){
        f = [NSString stringWithFormat:@"%.1fM", (float)fileSize/1048576];
    }
    else
    {
        f = [NSString stringWithFormat:@"%ldB",(long)fileSize];
    }
    return f;
}
+(NSString *)stringWithTimeString:(NSString *)times{
    
    NSDateFormatter *input = [[NSDateFormatter alloc]init];
    [input setDateFormat:@"yyyyMMddHHmmss"];
    NSDate* inputDate = [input dateFromString:times];
    [input setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString *timez = [input stringFromDate:inputDate];
    return timez;
}
